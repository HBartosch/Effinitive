# Simple test script for EffinitiveFramework + Entity Framework Core Sample
...
Write-Host "1. GET /api/products (Get all products)" -ForegroundColor Yellow
...
$products | Select-Object -First 3 | Format-Table id, name, category, price, stock -AutoSize
Write-Host ""

# Test 2: Get product by ID
Write-Host "2. GET /api/products/1 (Get specific product)" -ForegroundColor Yellow
...
Write-Host "3. GET /api/products/category/Electronics" -ForegroundColor Yellow
...
Write-Host "4. POST /api/products (Create new product)" -ForegroundColor Yellow
$newProduct = @{
    name = "Gaming Headset"
...
Write-Host "5. PUT /api/products/$($created.id) (Update product)" -ForegroundColor Yellow
$updatedProduct = @{
    id = $created.id
    product = @{
...
Write-Host "6. POST /api/orders (Create new order)" -ForegroundColor Yellow
$newOrder = @{
...
Write-Host "7. GET /api/orders/$($order.id) (Get order details)" -ForegroundColor Yellow
...
Write-Host "8. GET /api/orders/customer/john.doe@example.com" -ForegroundColor Yellow
...
Write-Host "9. PATCH /api/orders/$($order.id)/status (Update to Shipped)" -ForegroundColor Yellow
$statusUpdate = @{
    id = $order.id
    status = "Shipped"
} | ConvertTo-Json
...
Write-Host "10. DELETE /api/products/$($created.id) (Delete product)" -ForegroundColor Yellow
$deleteRequest = @{
    id = $created.id
} | ConvertTo-Json
...
Write-Host "11. Performance Test: 100x GET /api/products" -ForegroundColor Yellow
